...
class Peminjaman_model extends CI_Model {
	...
	public function get_peminjaman_all($start, $end)
	{

		$query = $this->db->query("SELECT id, id_tools FROM peminjaman_tools_detail LIMIT $start , $end")->result();
		return $query;
	}


	public function get_peminjaman_filter($search, $start, $end)
	{

		$query = $this->db->query("SELECT id, id_tools FROM peminjaman_tools_detail WHERE id_tools LIKE '%$search%' LIMIT $start, $end")->result();
		return $query;
	}

	public function get_peminjaman_total($search)
	{

		if($search == ""){
			$likes = "";
		}else{
			$likes = " WHERE id_tools LIKE '%$search%'";
		}

		$row = $this->db->query("SELECT count(*) as jml FROM peminjaman_tools_detail $likes")->row();
    ...
    public function get_peminjaman_detail($id){
        $query = $this->db->query("SELECT *, CONCAT(c.kode_rak, '', c.nomor_rak) as kode_rak ,
	...
	{	
		...
		$this->db->query("UPDATE peminjaman_tools SET aktif=0 WHERE id='$id'");
